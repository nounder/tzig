...
//   upstream: src/terminal/tmux/output.zig
...
pub fn FormatStruct(comptime vars: []const Variable) type {
    var field_names: [vars.len][]const u8 = undefined;
    var field_types: [vars.len]type = undefined;
    var field_attrs: [vars.len]std.builtin.Type.StructField.Attributes = undefined;
    for (vars, 0..) |variable, i| {
        field_names[i] = @tagName(variable);
        field_types[i] = variable.Type();
        field_attrs[i] = .{
            .@"comptime" = false,
            .@"align" = @alignOf(variable.Type()),
