...
//   upstream: src/terminal/StringMap.zig
...
test "StringMap searchIterator URL detection" {
    ...
    const testing = std.testing;
    const alloc = testing.allocator;
    // tzig: upstream imports `../config/url.zig` (ghostty's URL regex
    // config). With oniguruma=false this test is skipped at runtime, so
    // we comment out the import to keep the file parseable.
    const url = struct {
...
test "StringMap searchIterator URL with click position" {
    ...
    const testing = std.testing;
    const alloc = testing.allocator;
    // tzig: see prior test — same `../config/url.zig` stub.
    const url = struct {
        const regex: []const u8 = "";
    };
    // const url = @import("../config/url.zig");
