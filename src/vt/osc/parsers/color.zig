...
//   upstream: src/terminal/osc/parsers/color.zig
...
/// A list of color operation requests. Originally a SegmentedList
/// (removed in Zig 0.16); we use ArrayList here. The SegmentedList's
/// stable-pointer property is not relied on by current call sites
/// (each `addOne` result is filled in immediately), so this is a
/// drop-in for the parser-side semantics. Consumer-side iteration
/// still uses the SegmentedList API in the rest of the tree, which
/// will surface as a follow-up port task.
