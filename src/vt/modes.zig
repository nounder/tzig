...
//   upstream: src/terminal/modes.zig
...
pub const ModePacked = packed_struct: {
    var field_names: [entries.len][]const u8 = undefined;
    var field_types: [entries.len]type = undefined;
    var field_attrs: [entries.len]std.builtin.Type.StructField.Attributes = undefined;
...
pub const Mode = mode_enum: {
    var field_names: [entries.len][]const u8 = undefined;
    var field_values: [entries.len]ModeTag.Backing = undefined;
    for (entries, 0..) |entry, i| {
        field_names[i] = entry.name;
        field_values[i] = @as(ModeTag.Backing, @bitCast(ModeTag{
            .value = entry.value,
            .ansi = entry.ansi,
        }));
    }
