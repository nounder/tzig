...
pub fn TaggedUnion(
    ...
    return struct {
        ...
        pub const CValue = cvalue: {
            ...
            const N = tag_fields.len + 1;
            var field_names: [N][]const u8 = undefined;
            var field_types: [N]type = undefined;
            var field_attrs: [N]std.builtin.Type.UnionField.Attributes = undefined;
            for (tag_fields, 0..) |field, i| {
                ...
                field_types[i] = Type;
                field_attrs[i] = .{ .@"align" = @alignOf(Type) };
            }

            field_names[tag_fields.len] = "_padding";
            field_types[tag_fields.len] = Padding;
            field_attrs[tag_fields.len] = .{ .@"align" = @alignOf(Padding) };
