...
//   upstream: src/datastruct/blocking_queue.zig
//   commit:   c74f6d56d1feef473033057bc0ff7e3f00cf6421
//   pruned:   import paths flattened (../X -> X for sibling subtrees);
//            Mutex/Condition stubbed for Zig 0.16 (search.Thread is dead
//            code in the .lib build path, see PORT_LESSONS.md)
...
// 0.16 moved std.Thread.Mutex -> std.Io.Mutex (which requires an Io
// instance). The only consumer of BlockingQueue in upstream is
// search/Thread.zig, which is hard-set to `void` in tzig's .lib build
// (see search.zig). The tests below exercise only the non-blocking
// paths from a single thread, so we provide trivial stubs that satisfy
// the API surface without needing an Io.
const Mutex = struct {
...
const Condition = struct {
    pub fn wait(_: *Condition, _: *Mutex) void {}
