...
//   upstream: src/datastruct/split_tree.zig
//   commit:   c74f6d56d1feef473033057bc0ff7e3f00cf6421
//   pruned:   import paths flattened (../X -> X for sibling subtrees)

const std = @import("std");
const assert = @import("../quirks.zig").inlineAssert;
// tzig: dropped `../build_config.zig` import (ghostty app config, not
// vendored). It only fed the GTK gobject path which we elide below.
...
pub fn SplitTree(comptime V: type) type {
    return struct {
        ...
        /// tzig: GTK path elided; we don't vendor gobject/glib bindings.
        pub const getGObjectType = void;
        // unreachable, kept for diff reference:
        // pub const getGObjectType_unused = switch (build_config.app_runtime) {
