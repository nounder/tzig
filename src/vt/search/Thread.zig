...
//   upstream: src/terminal/search/Thread.zig
...
// tzig: upstream's threaded search ties into ghostty's libxev wrapper
// (`../../global.zig`) and ghostty's os subtree (`../os/main.zig`).
// Both are not vendored. The file remains compiled to keep the search
// package's surface intact, but `search.Thread` is hard-set to `void`
// for artifact=.lib so this code is unreachable. Stub the imports so
// the file parses.
const xev = struct {};
const internal_os = struct {};
