...
//   upstream: src/terminal/Terminal.zig
                ...
                .wide => wide: {
                    ...
                    if (self.screens.active.cursor.x == right_limit - 1) {
                        ...
                        if (prev.cell.hasGrapheme()) {
                            ...
                            try self.printWrap();
                            self.printCell(@intCast(prev_cp), .wide);
...
test "Terminal: fullReset tracked pins" {
    // pinIsValid uses comptime assert(slow_runtime_safety); only meaningful
    // in slow-runtime-safety builds, so gate the entire test on that flag.
