...
pub const Command = union(Key) {
    ...
    // 0.16-api: size assertion removed because ArrayList(T) (replacement
    // for SegmentedList in 0.16) is wider than the old SegmentedList,
    // changing Command's size. The original assertion documented an
    // intentional size budget; the new size needs to be re-measured.
};
