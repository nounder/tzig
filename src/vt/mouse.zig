...
//   upstream: src/terminal/mouse.zig
...
pub const Shape = enum(c_int) {
    ...
    /// tzig: upstream returns a gobject type when running under GTK
    /// (artifact=.ghostty + app_runtime=.gtk). We always build with
    /// artifact=.lib, so the gobject path is unreachable. Force `void`
    /// and drop the `../build_config.zig` and `gobject` imports.
...
test {
    // 0.16 removed refAllDeclsRecursive; non-recursive is sufficient
    // for keeping decls referenced.
