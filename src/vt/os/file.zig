...
// tzig: dropped Windows support — `windows.zig` (vendored from
// upstream src/os/windows.zig) was removed since tzig doesn't run
// on Windows. The `if (builtin.os.tag == .windows)` branches below
// are unreachable in our build but kept (with stubbed names) for
// minimal diff against upstream.
const windows = struct {
    const MAX_PATH: usize = 0;
    const exp = struct {
        const kernel32 = struct {
            fn GetTempPathW(_: anytype, _: anytype) u32 {
                unreachable;
            }
        };
    };
