...
//   upstream: src/terminal/page.zig
...
pub const Cell = packed struct(u64) {
    ...
    ///
    /// In Zig 0.16 packed unions require all variants to share the
    /// same bit width. RGB is 24 bits, so codepoint and color_palette
    /// are widened to u24 (the underlying values still occupy only
    /// u21 / u8 respectively; the extra bits are unused). Cells are
    /// always zero-initialized via @bitCast(@as(u64, 0)) before any
    /// variant is written, so the high bits remain zero.
    ...
    pub inline fn codepoint(self: Cell) u21 {
        return switch (self.content_tag) {
            .codepoint,
            .codepoint_grapheme,
            // codepoint is now u24 to satisfy 0.16's packed-union
            // equal-width rule; truncate back down to the valid
            // Unicode range we always store there.
