...
//   upstream: src/terminal/main.zig
...
// Upstream form preserved: this short-circuits to an empty struct when
// tmux_control_mode is off. The `tmux/control.zig` import depends on
// the `oniguruma` Zig package, which we don't pull in (oniguruma=false),
// so we rely on the comptime branch to skip importing tmux.zig at all.
...
// tzig: original pattern was
//   pub const c_api = if (options.c_abi) @import("c/main.zig") else void;
// Zig 0.16 still resolves the @import even in the false branch, and we
// pruned the C ABI subtree (terminal_options.c_abi = false). Hard-set void.
pub const c_api = void;
